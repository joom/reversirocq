...
struct WebGame {
  ...
  std::optional<Reversirocq::loop_state> loop;
...
void web_frame(void *arg) {
  ...
  auto result = Reversirocq::process_frame(game->ren, std::move(*game->loop));
  game->loop = std::move(result.second);

  if (result.first) {
    Reversirocq::cleanup(game->ren, game->win);
...
int main() {
  ...
  auto init = Reversirocq::init_game();
